...
export default class basic_functions {
  static get_people_from_appointment(appointment, allUsers) {
    ...
    const scheduler = allUsers.find(
      (user) => user.firebase_id === appointment.scheduler_id,
    );
    const appointee = allUsers.find(
      (user) => user.firebase_id === appointment.appointee_id,
    );
    return {
      scheduler,
      appointee,
  ...
  static async change_status(
     ...
     */
    const { scheduler, appointee } = this.get_people_from_appointment(
      appointment,
      allUsers,
    );
      ...
      .post(
        `${baseUrl}/update-appointment`,
        {
          ...
          scheduler_status_notif_pending:
            appointment.scheduler_status_notif_pending,
          appointee_status_notif_pending:
            appointment.appointee_status_notif_pending,
        },
        {
          headers: {
            Authorization: `Bearer ${userToken}`,
  ...
  static getAppointments(userSchedule, given_date) {
    ...
    for (let i = 0; i < userSchedule?.taken_appointments?.length; i++) {
      const appointmentDate =
        userSchedule.taken_appointments[i].appointment_date;
      const parsedGivenDate = parse(given_date, "yyyy-MM-dd", new Date());
      const parsedAppointmentDate = parse(
        appointmentDate,
        "yyyy-MM-dd",
        new Date(),
      );
    ...
    for (let i = 0; i < userSchedule?.given_appointments?.length; i++) {
      const appointmentDate =
        userSchedule.given_appointments[i].appointment_date;
      const parsedGivenDate = parse(given_date, "yyyy-MM-dd", new Date());
      const parsedAppointmentDate = parse(
        appointmentDate,
        "yyyy-MM-dd",
  ...
  static async get_user_schedule_for_week(
    firebase_id,
    base_url,
    userToken,
    currentWeek,
  ...
  static get_current_month_week(currentWeek) {
    ...
    const suffix =
      ["st", "nd", "rd"][((((weekOfMonth + 90) % 100) - 10) % 10) - 1] || "th";
